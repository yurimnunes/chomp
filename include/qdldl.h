...
public:
    explicit FactorizationError(const std::string& msg) : std::runtime_error(msg) {}
};
class InvalidMatrixError : public std::runtime_error {
public:
    explicit InvalidMatrixError(const std::string& msg) : std::runtime_error(msg) {}
};
...
class SparseMatrix {
    ...
    SparseMatrix(IntType n, std::vector<IntType> col_ptr,
        ...
        : col_ptr_(std::move(col_ptr)),
          ...
          n_(n)
    {
        if (n_ < 0) throw InvalidMatrixError("n must be nonnegative");
        if (col_ptr_.size() != static_cast<std::size_t>(n_ + 1)) {
            throw InvalidMatrixError("Ap size must be n+1");
        }
        const std::size_t nnz = static_cast<std::size_t>(col_ptr_[static_cast<std::size_t>(n_)]);
    ...
    [[nodiscard]] std::span<const IntType>   col_ptr() const noexcept { return col_ptr_; }
    [[nodiscard]] std::span<const IntType>   row_idx() const noexcept { return row_idx_; }
    [[nodiscard]] std::span<const FloatType> values()  const noexcept { return values_; }

    // Friends for permutation builder
    template<FloatingPoint FT, SignedInteger IT>
    friend SparseMatrix<FT,IT> make_csc(IntType n,
                                        std::vector<IntType>&& col_ptr,
                                        std::vector<IntType>&& row_idx,
        ...
        for (IntType j = 0; j < n_; ++j) {
            ...
            for (IntType p = p0; p < p1; ++p) {
                ...
                if (QDLL_UNLIKELY(i > j)) {
                    throw InvalidMatrixError("Lower-triangular entry found (expect upper+diag only)");
                }
            }
        }
    }
...
template<FloatingPoint FT, SignedInteger IT>
inline SparseMatrix<FT,IT> make_csc(IT n,
                                    std::vector<IT>&& col_ptr,
...
struct LDLFactorization {
    ...
    explicit LDLFactorization(IntType n)
        : L_col_ptr(static_cast<std::size_t>(n + 1)),
          L_row_idx(),
          L_values(),
          D(static_cast<std::size_t>(n)),
          D_inv(static_cast<std::size_t>(n)),
...
// ===================== Ordering (perm hooks) =====================
template<SignedInteger IntType = std::int64_t>
struct Ordering {
...
inline SparseMatrix<FT,IT> permute_symmetric_upper(const SparseMatrix<FT,IT>& A,
                                                   const Ordering<IT>& ord)
{
    const IT n = A.size();
    if (ord.n != n) throw InvalidMatrixError("Ordering size mismatch");
    ...
    for (IT j = 0; j < n; ++j) {
        ...
        for (IT p = p0; p < p1; ++p) {
            ...
            if (pi <= pj) {
                ...
                counts[static_cast<std::size_t>(pi)]++; // move to the other column's upper
            }
        }
    }
    ...
    for (IT j = 0; j < n; ++j) Cp[static_cast<std::size_t>(j + 1)] = Cp[static_cast<std::size_t>(j)] + counts[static_cast<std::size_t>(j)];
    ...
    for (IT j = 0; j < n; ++j) {
        ...
        for (IT p = p0; p < p1; ++p) {
            ...
            IT col = pj;
            IT row = pi;
            if (row > col) { std::swap(row, col); } // push to upper
    ...
    for (IT j = 0; j < n; ++j) {
        ...
        for (IT p = p0 + 1; p < p1; ++p) {
            ...
            while (q > p0 && Ci[static_cast<std::size_t>(q - 1)] > ri) {
                Ci[static_cast<std::size_t>(q)] = Ci[static_cast<std::size_t>(q - 1)];
                Cx[static_cast<std::size_t>(q)] = Cx[static_cast<std::size_t>(q - 1)];
                --q;
            }
...
public:
    ...
    [[nodiscard]] static auto compute_etree(const SparseMatrix<FloatType, IntType>& A)
        -> std::pair<std::vector<IntType>, std::vector<IntType>>
    {
        ...
        std::vector<IntType> etree(static_cast<std::size_t>(n), UNKNOWN<IntType>);
        ...
        for (IntType j = 0; j < n; ++j) {
            ...
            for (IntType p = Ap[static_cast<std::size_t>(j)]; p < p_end; ++p) {
                ...
                if (QDLL_UNLIKELY(i > j)) {
                    throw InvalidMatrixError("Lower-triangular entry encountered in etree()");
                }
                while (work[static_cast<std::size_t>(i)] != j) {
                    if (etree[static_cast<std::size_t>(i)] == UNKNOWN<IntType>) {
    ...
    analyze(const SparseMatrix<FloatType, IntType>& A)
    {
        ...
        for (IntType i = 0; i < S.n; ++i) {
            S.L_col_ptr[static_cast<std::size_t>(i + 1)] =
                S.L_col_ptr[static_cast<std::size_t>(i)] + S.Lnz[static_cast<std::size_t>(i)];
        }
        return S;
    }
                ...
                const Symbolic<FloatType,IntType>& S)
    {
        ...
        for (IntType i = 0; i < n; ++i) Lnext[static_cast<std::size_t>(i)] = Lp[static_cast<std::size_t>(i)];
        ...
        for (IntType k = 0; k < n; ++k) {
            ...
            for (IntType ip = Ap[static_cast<std::size_t>(k)]; ip < pend; ++ip) {
                const IntType b_idx = Ai[static_cast<std::size_t>(ip)];
                if (b_idx == k) {
                    D[static_cast<std::size_t>(k)] = Ax[static_cast<std::size_t>(ip)];
                    diag_seen = true;
                    continue;
                }
                yVals[static_cast<std::size_t>(b_idx)] = Ax[static_cast<std::size_t>(ip)];
            ...
            while (nnz_y > 0) {
                ...
                for (IntType j = j0; j < j1; ++j) {
                    yVals[static_cast<std::size_t>(Li[static_cast<std::size_t>(j)])]
            ...
            if (D[static_cast<std::size_t>(k)] > FloatType{0}) ++R.positive_eigenvalues;
            Dinv[static_cast<std::size_t>(k)] = FloatType{1} / D[static_cast<std::size_t>(k)];
        }
        return R;
    }
    ...
    {
        solve_L(L, x);
        for (std::size_t i = 0; i < L.D_inv.size(); ++i) x[i] *= L.D_inv[i];
        solve_Lt(L, x);
    }
                             ...
                             int iters = 2)
    {
        ...
        if (b.size() != n) throw std::invalid_argument("b/x size mismatch");
        ...
        for (int it = 0; it < iters; ++it) {
            ...
            // y = D y
            for (std::size_t i = 0; i < n; ++i) y[i] *= L.D[i];

            // y = (L+I)^T y
            solve_Lt(L, std::span<FloatType>(y));

            // r -= y
            for (std::size_t i = 0; i < n; ++i) r[i] -= y[i];
            ...
            // x += t
            for (std::size_t i = 0; i < n; ++i) x[i] += t[i];
        }
    }
};
