...
public:
    ...
    struct DegeneracySignals {
        ...
        std::unordered_map<int, std::vector<double>> lex_order_abs;// abs -> lex tuple
    ...
    begin_pricing(double objective,
        ...
        if (deg_streak_ >= 3) {
            ...
            // Fill “shape” hints keyed by pseudo-abs 0..(nN_-1); bridge will rewrite keys to ABS.
            // If you prefer, you can leave this empty and let pricer do its own lex.
            for (int kRel = 0; kRel < nN_; ++kRel) {
                // store by pseudo key = kRel; bridge will remap using N[kRel]
                signals_.lex_order_abs[kRel] = {static_cast<double>(kRel), eps(rng_)};
            }
        }

        // Ill-conditioning → gently increase weights (more conservative), keyed by pseudo
        if (cond_est_ > 1e10) {
            for (int kRel = 0; kRel < nN_; ++kRel) {
                signals_.weight_overrides_abs[kRel] = 2.0; // heavier (bridge remaps)
            }
        }

        // Recurrent suspects (forbid), only if degeneracy persists — ABS ids already tracked
        trim_repeat_window_();
    ...
    // (Deprecated) Optional filter for REL candidates — no-op now that signals are ABS-only.
    void filter_candidates_in_place(std::vector<int> &/*rel_candidates*/) const {
        // Intentionally empty: filtering happens in the bridge using ABS→REL mapping.
    }
    ...
    void after_pivot(int /*leaving_rel*/,
        ...
        if (std::abs(step_alpha) <= dm_consts::kDegenerateAlphaTol) { // degenerate pivot
            ++deg_streak_;
            ++deg_total_;
...
private:
    ...
    void push_step_(double s) {
        if ((int)step_hist_.size() >= dm_consts::kStepHistCap) step_hist_.pop_front();
        step_hist_.push_back(s);
    }
    bool small_recent_improvement_() const {
        const bool obj_stalled = (last_obj_impr_ < dm_consts::kObjStallTol);
        const bool rc_stalled  = (std::abs(last_rc_impr_) < dm_consts::kRcStallTol);
    ...
    void push_repeat_(int entering_abs) {
        ...
        repeat_abs_block_.clear();
        for (auto &kv : repeat_freq_) if (kv.second >= dm_consts::kRepeatMinCount)
    ...
    void trim_repeat_window_() {
        if (deg_streak_ == 0 && !repeat_window_.empty() && (int)repeat_window_.size() > 8) {
...
struct DegeneracyPricerBridge {
    ...
    choose_entering(const Eigen::VectorXd &rN, const std::vector<int> &N,
        ...
        auto entering_rel =
            pricer.choose_entering(rN_eff, N, tol, iteration, current_objective, basis, A);
            ...
            if (forbid.count(N[*entering_rel])) {
                ...
                for (int k = 0; k < (int)N.size(); ++k) {
                    if (rN_eff(k) < -tol && !forbid.count(N[k]) && rN_eff(k) < best_rc) {
                        best_rc = rN_eff(k); best = k;
                    }
                }
                if (best >= 0) entering_rel = best;
                else entering_rel.reset();
            }
        }
        return entering_rel;
    }
...
void after_pivot(int leaving_rel, int entering_abs, int old_abs,
    ...
    // Call matches AdaptivePricer signature (step_size passed, no trailing bool)
...
public:
    ...
    void update_after_pivot(int leave_rel, int e_abs, int old_abs,
        ...
        if (std::abs(alpha) < dm_consts::kDegenerateAlphaTol) { need_rebuild_ = true; return; }
    ...
    void adapt_strategy_() {
        ...
        const double recent_avg =
            std::accumulate(recent_objectives_.end() - dm_consts::kPerfWindow,
                            recent_objectives_.end(), 0.0) / dm_consts::kPerfWindow;
        const double older_avg  =
            std::accumulate(recent_objectives_.begin(),
                            recent_objectives_.begin() + dm_consts::kPerfWindow, 0.0) / dm_consts::kPerfWindow;
