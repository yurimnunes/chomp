...
inline const char* to_string(LPSolution::Status s) {
    switch (s) {
        ...
        case LPSolution::Status::IterLimit:  return "iterlimit";
        case LPSolution::Status::Singular:   return "singular";
        case LPSolution::Status::NeedPhase1: return "need_phase1";
    }
    return "unknown";
}
...
public:
    ...
    LPSolution solve(const Eigen::MatrixXd &A_in,
                     ...
                     std::optional<std::vector<int>> basis_opt = std::nullopt)
    {
        ...
        if (pres.proven_unbounded) {
            Eigen::VectorXd xnan = Eigen::VectorXd::Constant(n, std::numeric_limits<double>::quiet_NaN());
        ...
        if (Atil.rows() == 0) {
            ...
            for (int j = 0; j < static_cast<int>(ctil.size()); ++j) {
                if (ctil(j) > opt_.tol) {
                    ...
                    if (std::isfinite(ured(j))) vred(j) = ured(j);
                    else { is_bounded = false; break; }
                } else {
                    vred(j) = std::isfinite(lred(j)) ? lred(j) : 0.0;
                }
            }
            ...
            return make_solution_(LPSolution::Status::Optimal, x_full, total_obj,
        ...
        Eigen::VectorXd u_eff = Eigen::VectorXd::Constant(n_eff, presolve::inf());
        ...
        if (basis_opt && !basis_opt->empty()) {
            std::unordered_map<int,int> orig2red; orig2red.reserve(n_eff);
            for (int jr = 0; jr < n_eff; ++jr) {
                const int jorig = col_orig_map[jr];
                if (jorig >= 0) orig2red[jorig] = jr;
            }
            std::vector<int> cand; cand.reserve(std::min(m_eff, (int)basis_opt->size()));
        ...
        // ---- (5) Try Phase II directly on reduced problem (Primal/Dual per mode) ----
        std::vector<int> basis_guess;
        if (red_basis_opt && (int)red_basis_opt->size() == m_eff) {
            basis_guess = *red_basis_opt;
        } else {
            if (auto maybe = find_initial_basis_(Ared, bred)) basis_guess = *maybe;
        }
        ...
        if ((int)basis_guess.size() == m_eff) {
            ...
            if (opt_.mode == SimplexMode::Dual) {
                ...
                std::tie(st, v2, red_basis2, it2, info2) = run_primal();
                if (st == LPSolution::Status::NeedPhase1 && info2.count("reason") &&
                ...
                st == LPSolution::Status::IterLimit)
            {
                ...
                std::vector<int> basis_full; basis_full.reserve(red_basis2.size());
                ...
                return make_solution_(st, x_full, total_obj, basis_full, it2, std::move(info));
            }
        ...
        std::vector<int> red_basis2; red_basis2.reserve(m_rows);
        for (int j : basis1_out) if (j < (int)n_orig_eff) red_basis2.push_back(j);
        ...
        std::vector<int> basis_full; basis_full.reserve(red_basis_out.size());
...
private:
    ...
    static Eigen::VectorXd clip_small_(Eigen::VectorXd x, double tol = 1e-12) {
        for (int i = 0; i < x.size(); ++i) if (std::abs(x(i)) < tol) x(i) = 0.0;
        return x;
    }
    ...
    find_initial_basis_(const Eigen::MatrixXd &A, const Eigen::VectorXd &b) {
        ...
        std::vector<int> basis; basis.reserve(m);
        ...
        for (int j = 0; j < n; ++j) {
            ...
            for (int i = 0; i < m; ++i) {
                ...
                if (std::abs(v - 1.0) <= 1e-12) { one_idx = i; ++ones; }
                else if (std::abs(v) > 1e-12)  { zeros_ok = false; break; }
            }
            if (ones == 1 && zeros_ok && !used_row[one_idx] && b(one_idx) >= -1e-12) {
                basis.push_back(j);
                used_row[one_idx] = true;
                if ((int)basis.size() == m) break;
            }
        }
        if ((int)basis.size() == m) return basis;
        return std::nullopt;
    }
    ...
    make_phase1_(const Eigen::MatrixXd &A, const Eigen::VectorXd &b) {
        ...
        Eigen::MatrixXd A1 = A;
        Eigen::VectorXd b1 = b;
        for (int i = 0; i < m; ++i) if (b1(i) < 0) { A1.row(i) *= -1.0; b1(i) *= -1.0; }

        Eigen::MatrixXd A_aux(m, n + m);
        A_aux.leftCols(n)  = A1;
        A_aux.rightCols(m) = Eigen::MatrixXd::Identity(m, m);

        Eigen::VectorXd c_aux(n + m); c_aux.setZero(); c_aux.tail(m).setOnes();
                             ...
                             const Eigen::VectorXd &u_in)
    {
        ...
        std::vector<int> ub_idx; ub_idx.reserve(n0);
        constexpr double kMaxReasonableUB = 1e6;
        for (int j = 0; j < n0; ++j) if (std::isfinite(u_in(j)) && u_in(j) <= kMaxReasonableUB) ub_idx.push_back(j);
        ...
        Eigen::MatrixXd A(m0 + p, n0 + p); A.setZero();
    ...
    harris_ratio_(const Eigen::VectorXd &xB,
                  const Eigen::VectorXd &dB,
                  double delta, double eta) const
    {
        std::vector<int> pos; pos.reserve(dB.size());
        for (int i = 0; i < dB.size(); ++i) if (dB(i) > delta) pos.push_back(i);
        if (pos.empty()) return {std::nullopt, std::numeric_limits<double>::infinity()};

        double theta_star = std::numeric_limits<double>::infinity();
        for (int idx : pos) theta_star = std::min(theta_star, xB(idx) / dB(idx));

        double max_resid = 0.0;
        std::vector<int> candidates;
        for (int idx : pos) {
            const double ratio = xB(idx) / dB(idx);
            if (std::abs(ratio - theta_star) <= 1e-10) candidates.push_back(idx);
            const double resid = xB(idx) - theta_star * dB(idx);
            max_resid = std::max(max_resid, std::max(0.0, resid));
        }
        if (!candidates.empty()) {
            int best = candidates.front();
            for (int idx : candidates) if (idx < best) best = idx;
        ...
        if (!eligible.empty()) {
            ...
            for (int idx : eligible) if (idx < best) best = idx;
            return {best, theta_star};
        }
        ...
        for (int i = 1; i < (int)pos.size(); ++i) {
            ...
            if (r < best_ratio) { best_ratio = r; best = idx; }
        }
        return {best, best_ratio};
    }

    // BFRT: permissible step for entering variable to nearest active bound (primal)
    struct BFRTStep { double theta_e = std::numeric_limits<double>::infinity(); bool to_upper = false; };

    BFRTStep entering_bound_step_(double x_e, double l_e, double u_e,
                                  double rc_e, double tol) const
    {
        BFRTStep out;
        // Primal simplex (minimization): rc_e < 0 ⇒ objective improves as x_e increases
        if (rc_e < -tol) {
            if (std::isfinite(u_e)) { out.theta_e = std::max(0.0, u_e - x_e); out.to_upper = true; }
        } else if (rc_e > tol) { // move downward
            if (std::isfinite(l_e)) { out.theta_e = std::max(0.0, x_e - l_e); out.to_upper = false; }
        }
        return out;
    }
           ...
           const Eigen::VectorXd &u)
    {
        ...
        if (basis_opt) {
            basis = *basis_opt;
            if ((int)basis.size() != m)
                return {LPSolution::Status::NeedPhase1, Eigen::VectorXd::Zero(n), {},
                        0, {{"reason","basis size != m"}}};
        } else {
            auto maybe = find_initial_basis_(A, b);
            if (!maybe)
                return {LPSolution::Status::NeedPhase1, Eigen::VectorXd::Zero(n), {},
                        0, {{"reason","no_trivial_basis"}}};
            basis = *maybe;
        }

        // Nonbasic list N
        std::vector<int> N; N.reserve(n - m);
        {
            std::vector<char> inB(n, 0);
            for (int j : basis) {
                if (j < 0 || j >= n)
                    return {LPSolution::Status::Singular, Eigen::VectorXd::Zero(n),
                            basis, 0, {{"where","initial basis index out of range"}}};
                inB[j] = 1;
            }
        ...
        FTBasis B(A, basis, opt_.refactor_every, opt_.compress_every,
                  opt_.lu_pivot_rel, opt_.lu_abs_floor, opt_.alpha_tol, opt_.z_inf_guard);
        ...
        if (opt_.pricing_rule == "adaptive") {
            ...
            bridge_ = std::make_unique<DegeneracyPricerBridge<AdaptivePricer>>(degen_, adaptive_pricer_);
        }
        ...
        while (iters < opt_.max_iters) {
            ...
            if (opt_.bland) {
                ...
                if (idx < 0) {
                    ...
                    return {LPSolution::Status::Optimal, clip_small_(x), basis, iters,
                ...
                if (opt_.pricing_rule == "adaptive") {
                    ...
                    for (int i = 0; i < m; ++i) xcur(basis[i]) = xB(i);
                    const double current_obj = c.dot(xcur);

                    e_rel = bridge_->choose_entering(rN, N, opt_.tol, iters, current_obj, B, A);
                } else {
                    // Most negative rc
            ...
            const double x_e  = std::isfinite(l_e) ? l_e : 0.0; // nonbasic at bound
            const BFRTStep bfrt = entering_bound_step_(x_e, l_e, u_e, rc_e, opt_.tol);
            ...
            // Degeneracy signals
            const bool is_degenerate = degen_.detect_degeneracy(step, opt_.deg_step_tol);
            if (is_degenerate && degen_.should_apply_perturbation()) {
                auto [Ap, bp, cp] = degen_.apply_perturbation(A, b, c, basis, iters);
                (void)Ap; (void)bp; (void)cp; // no-op by default, preserves API
            } else {
            ...
            if (opt_.pricing_rule == "adaptive") {
                ...
                bridge_->after_pivot(r, eAbs, oldAbs, dB, alpha, step, A, N, rc_impr);
            }
            ...
            if (opt_.pricing_rule == "adaptive" && adaptive_pricer_.needs_rebuild()) {
                adaptive_pricer_.build_pools(B, A, N);
                adaptive_pricer_.clear_rebuild_flag();
            }
        }
    ...
    //   pN: row r of B^{-1}A_N
    // Eligibility: pN(k) < -delta (so increasing τ ≥ 0 increases x_r by -τ pN(k))
    struct DualChoose {
        std::optional<int> e_rel;
    ...
    DualChoose dual_harris_choose_(const Eigen::VectorXd &rN,
                                   ...
                                   double delta, double eta) const
    {
        std::vector<int> E; E.reserve((int)pN.size());
        for (int k = 0; k < pN.size(); ++k) if (pN(k) < -delta) E.push_back(k);
        if (E.empty()) return {};
        ...
        if (!candidates.empty()) {
            ...
            for (int kk : candidates) {
                ...
                if ( (val < best_ratio - 1e-16) || (std::abs(val - best_ratio) <= 1e-16 && kk < best) ) {
                    best = kk; best_ratio = val;
                }
            }
        ...
        for (int i = 1; i < (int)E.size(); ++i) {
            ...
            if (val < best_ratio) { best_ratio = val; best = k; }
        }
        return {best, std::max(0.0, best_ratio)};
    }
    ...
    std::tuple<LPSolution::Status, Eigen::VectorXd, std::vector<int>, int,
               std::unordered_map<std::string,std::string>>
    dual_phase_(const Eigen::MatrixXd &A,
                const Eigen::VectorXd &b,
                const Eigen::VectorXd &c,
                std::optional<std::vector<int>> basis_opt,
                const Eigen::VectorXd &l,
                const Eigen::VectorXd &u)
    {
        (void)l; (void)u; // bounds handled at presolve; dual phase here is standard
        ...
        while (iters < opt_.max_iters) {
            ...
            // If all xB >= 0 (within tol), primal feasible; we just need dual feasibility
            // Compute dual vector y and reduced costs rN; if rN >= -tol, optimal.
            int r_leave = -1;
            double most_neg = -opt_.tol;
            for (int i = 0; i < m; ++i) {
                if (xB(i) < most_neg) { most_neg = xB(i); r_leave = i; }
            }
            if (r_leave < 0) {
                // Check dual feasibility: y = B^{-T} c_B ; rN = c_N - A_N^T y
                Eigen::VectorXd cB(m);
                for (int i = 0; i < m; ++i) cB(i) = c(basis[i]);
                Eigen::VectorXd y;
                try {
                ...
                for (int k = 0; k < (int)N.size(); ++k) {
                    const int j = N[k];
                    const double rj = c(j) - A.col(j).dot(y);
                    if (rj < -opt_.tol) { dual_feasible = false; break; }
                }
                if (dual_feasible) {
                    // Recover primal x
                    Eigen::VectorXd x = Eigen::VectorXd::Zero(n);
                ...
                // If dual infeasible while primal feasible with current basis—should not happen with this loop.
                // Fall back to primal path to polish.
                return {LPSolution::Status::NeedPhase1, Eigen::VectorXd::Zero(n), basis, iters,
                        {{"reason","dual_infeasible_at_primal_feasible"}}};
            }

            // Build dual pricing row: w = B^{-T} e_r; then pN = w^T A_N
            Eigen::VectorXd e_r = Eigen::VectorXd::Zero(m); e_r(r_leave) = 1.0;
            Eigen::VectorXd w;
            try {
                w = B.solve_BT(e_r);
            } catch (...) {
            ...
            // Choose entering with dual Harris rule
            DualChoose dc = dual_harris_choose_(rN, pN, opt_.ratio_delta, opt_.ratio_eta);
            if (!dc.e_rel) {
                // No eligible entering for this leaving ⇒ likely singular or infeasible
                if (rebuild_attempts < opt_.max_basis_rebuilds) {
                    ++rebuild_attempts;
                    B.refactor();
                    if (opt_.pricing_rule == "adaptive") {
            ...
            // Step τ updates reduced costs of nonbasics; primal variable x_r increases by τ
            ...
            if (!std::isfinite(tau)) {
                ...
                return {LPSolution::Status::Infeasible, Eigen::VectorXd::Zero(n), basis, iters,
                        {{"where","dual: infinite step"}}};
            }

            // Degeneracy tracking: treat tiny tau as degenerate
            const bool is_degenerate = degen_.detect_degeneracy(tau, opt_.deg_step_tol);
            if (is_degenerate && degen_.should_apply_perturbation()) {
                auto [Ap, bp, cp] = degen_.apply_perturbation(A, b, c, basis, iters);
            ...
            if (opt_.pricing_rule == "adaptive") {
                // Use pN and tau magnitude as signal
                const double rc_impr = tau; // proxy
                bridge_->after_pivot(r_leave, eAbs, basis[r_leave], pN, pN(e_rel), tau, A, N, rc_impr);
            }

            // Perform pivot: replace column r_leave by a_e
            const auto aE = A.col(eAbs);
            const int oldAbs = basis[r_leave];
            basis[r_leave] = eAbs;
            N[e_rel] = oldAbs;

            try {
                B.replace_column(r_leave, aE);
            } catch (...) {
                B.refactor();
                if (opt_.pricing_rule == "adaptive") {
                    adaptive_pricer_.build_pools(B, A, N);
    ...
    // --------------------------- Utilities ---------------------------
    static LPSolution make_solution_(LPSolution::Status st,
                                     Eigen::VectorXd x,
                                     double obj,
                                     std::vector<int> basis,
                                     int iters,
