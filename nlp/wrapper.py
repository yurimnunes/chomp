...
class Expr:
    ...
    # nlp/wrapper.py  — PATCH 1
    @staticmethod
    def from_var(name: str, sl: slice, shape: Tuple[int, ...]) -> "Expr":
        # NOTE: env[name] already contains the per-variable block with correct shape,
        # as created by Model.build.pack_env(...). Do NOT apply 'sl' again here.
        def _fn(env):
            val = env[name]
            # if scalar, return scalar; else ensure correct shape
            if shape == ():
                # env[name] is a 0-dim scalar already
                return (
                    val
                    if _is_scalar_like(val)
                    else np.array(val, dtype=object).reshape(())
                )
            # ensure object array with desired shape
            arr = np.array(val, dtype=object)
            # tolerate already-correct shape
            if arr.shape != shape:
    ...
    def _binop(self, other: Any, op: Callable[[Any, Any], Any], bcast=True) -> "Expr":
        ...
        out_shape = (
            ...
            if not bcast
                ...
                self.shape if self.shape else (), b.shape if b.shape else ()
            )
        )
    ...
    def T(self) -> "Expr":
        out_shape = (
            (self.shape[1], self.shape[0]) if len(self.shape) == 2 else self.shape
        )
        return Expr(lambda env: self.eval(env).T, out_shape)

    def norm2(self) -> "Expr":
        return Expr(
            lambda env: _sqrt_ad_aware(_sum_ad_aware(self.eval(env) * self.eval(env))),
            (),
        )
    ...
    def __le__(self, rhs) -> "Constr":
        return Constr(self, "<=", rhs if isinstance(rhs, Expr) else Expr.constant(rhs))

    def __ge__(self, rhs) -> "Constr":
        return Constr(self, ">=", rhs if isinstance(rhs, Expr) else Expr.constant(rhs))

    def __eq__(self, rhs) -> "Constr":
        return Constr(self, "==", rhs if isinstance(rhs, Expr) else Expr.constant(rhs))  # type: ignore[override]
...
class Param(Expr):
    ...
    def __init__(self, name: str, shape: Tuple[int, ...]):
        ...
        # Pull directly from env[name], exactly like Var.from_var (no slicing)
        def _fn(env):
            val = env[name]
            if shape == ():
                return (
...
class Constr:
    def __init__(self, lhs: Expr, sense: str, rhs: Expr):
        ...
        self.shape = np.broadcast_shapes(
            lhs.shape if lhs.shape else (), rhs.shape if rhs.shape else ()
        )
...
class Model:
    ...
    def add_param(
        ...
        """Register a parameter symbol that can be set at runtime."""
        ...
        if value is not None:
            # store numeric or object array; keep shape
            self._param_values[name] = (
                np.array(value, dtype=object if AD else float).reshape(shape)
                if shape
                else (
                    value
                    if _is_scalar_like(value)
                    else np.array(value, dtype=object).reshape(())
                )
            )
        return p
    ...
    def set_param(self, name: str, value: ArrayLike):
        ...
        self._param_values[name] = (
            np.array(value, dtype=object if AD else float).reshape(shp)
            if shp
            else (
                value
                if _is_scalar_like(value)
    ...
    def set_params(self, mapping: Optional[Dict[str, ArrayLike]] = None, /, **kwargs):
        """Batch update: set_params({'A': A_val, 'B': B_val}, C=C_val)."""
        if mapping:
    ...
    def build(self):
        ...
        def pack_env(x_in) -> Dict[str, Any]:
            vec = _as_obj_array(x_in)
            # Start with a shallow copy of current param values so updates are visible
        ...
        for v in self._vars.values():
            if v.lb is not None:
                bound_cs.append(Constr(v, ">=", Expr.constant(v.lb)))
            if v.ub is not None:
                bound_cs.append(Constr(v, "<=", Expr.constant(v.ub)))
        ...
        def make_scalar_funcs(c: Constr):
            if c.sense == "<=":
                h = lambda env: c.lhs.eval(env) - c.rhs.eval(env)
                target = c_ineq
            elif c.sense == ">=":
                h = lambda env: c.rhs.eval(env) - c.lhs.eval(env)
                target = c_ineq
            else:
                h = lambda env: c.lhs.eval(env) - c.rhs.eval(env)
                target = c_eq

            # ✅ Use packed probe env so Expr.from_var slices line up
        ...
        for name in self._order:
            ...
            if name in self._x0_parts:
                ...
                if v.lb is not None and v.ub is not None:
                    part = (
                        np.array(v.lb, dtype=float) + np.array(v.ub, dtype=float)
                    ).reshape(-1) * 0.5
...
def _normalize_shape(shape: Union[int, Tuple[int, ...]]) -> Tuple[int, ...]:
    if shape == () or shape == 0 or shape == 1:
        return ()
