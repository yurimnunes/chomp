...
class InteriorPointStepper:
    ...
    def _ensure_cfg_defaults(self):
        ...
        # tolerances (linked to global)
        add("ip_tol_feas", getattr(self.cfg, "tol_feas", 1e-8))
        add("ip_tol_stat", getattr(self.cfg, "tol_stat", 1e-8))
        add("ip_tol_comp", getattr(self.cfg, "tol_comp", 1e-8))
        ...
        if not hasattr(self.cfg, "ls_backtrack") and hasattr(self.cfg, "ip_alpha_backtrack"):
            self.cfg.ls_backtrack = float(self.cfg.ip_alpha_backtrack)
        if not hasattr(self.cfg, "ls_armijo_f") and hasattr(self.cfg, "ip_armijo_coeff"):
    ...
    def step(
        ...
        cI = np.zeros(mI) if (mI == 0 or data["cI"] is None) else np.asarray(data["cI"], float)
        cE = np.zeros(mE) if (mE == 0 or data["cE"] is None) else np.asarray(data["cE"], float)
        ...
        if bool(getattr(self.cfg, "ip_exact_hessian", True)):
            ...
            H, _ = make_psd_advanced(H, self.reg, it, eig_floor=getattr(self.cfg, "ip_hess_reg0", 1e-4))

        # -------------------- residuals at x --------------------
        r_d = g.copy()
        if mI > 0: r_d += (JI.T @ lmb if sp.isspmatrix(JI) else np.asarray(JI).T @ lmb)
        if mE > 0: r_d += (JE.T @ nuv if sp.isspmatrix(JE) else np.asarray(JE).T @ nuv)
        r_pI = cI + s
        r_pE = cE

        # norms
        stat  = float(np.linalg.norm(r_d, ord=np.inf))
        feasI = 0.0 if mI == 0 else float(np.linalg.norm(np.maximum(0.0, cI), ord=np.inf))
        ...
        if (stat <= getattr(self.cfg, "ip_tol_stat", 1e-8) and
            max(feasI, feasE) <= getattr(self.cfg, "ip_tol_feas", 1e-8) and
            (0.0 if mI == 0 else abs(s @ lmb) / max(1, mI)) <= getattr(self.cfg, "ip_tol_comp", 1e-8) and
            mu <= getattr(self.cfg, "ip_mu_min", 1e-12)):
            info = self._pack_info(0.0, True, True, f, theta,
                                {"stat":stat,"ineq":feasI,"eq":feasE,"comp":comp},
                                1.0, 0.0, mu)
            return x, lmb, nuv, info

        # -------------------- build W and RHS --------------------
        def _isspm(A): return sp.isspmatrix(A)
        def _csr(A, shape=None):
            if A is None:
                if shape is None: raise ValueError("shape required when A is None")
                return sp.csr_matrix(shape)
            return A.tocsr() if _isspm(A) else sp.csr_matrix(A)
        def _sym(A):
        ...
        if mI > 0:
            ...
            if bool(getattr(self.cfg, "ip_match_ref_form", True)):
                ...
                if _isspm(H) or _isspm(JI):
                    Hs = _csr(H, (n, n)); JIcsr = _csr(JI, (mI, n))
                ...
                rhs_x = -r_d - ((JI.T @ rhs_core) if _isspm(JI) else (np.asarray(JI).T @ rhs_core))
            else:
                ...
                rhs_x_aff = -r_d - ((JI.T @ (D * r_pI)) if _isspm(JI) else (np.asarray(JI).T @ (D * r_pI)))
        else:
            W = _sym(H)
            rhs_x = -r_d

        # -------------------- solve KKT (JE block regularized) --------------------
        def solve_KKT(Wmat, rhs_x, JE_mat, rpE):
            if mE > 0:
                Wcsr  = _csr(Wmat, (n, n))
                JEcsr = _csr(JE_mat, (mE, n))
                B22   = float(getattr(self.cfg, "ip_eq_reg", 1e-4)) * sp.eye(mE, format="csr")
                K = sp.vstack([sp.hstack([Wcsr, JEcsr.T], format="csr"),
                            sp.hstack([JEcsr, B22],     format="csr")], format="csr")
                ...
                if _isspm(Wmat):
                    try:    dx = spla.spsolve(Wmat.tocsc(), rhs_x)
                    except: dx = np.linalg.solve(Wmat.toarray(), rhs_x)
                else:
                    dx = np.linalg.solve(Wmat, rhs_x)
                return dx, np.zeros(0)

        if mI == 0:
            dx, dnu = solve_KKT(W, rhs_x, JE, r_pE)
            ds = np.zeros(0); dlam = np.zeros(0)
        else:
            if bool(getattr(self.cfg, "ip_match_ref_form", True)):
                dx, dnu = solve_KKT(W, rhs_x, JE, r_pE)
                ...
                dx_aff, dnu_aff = solve_KKT(W, rhs_x_aff, JE, r_pE)
                JI_dx_aff = (JI @ dx_aff) if not _isspm(JI) else JI.dot(dx_aff)
                ds_aff = -r_pI - JI_dx_aff
                dl_aff = (lmb / s) * (r_pI + JI_dx_aff) - lmb
                a_p_aff = self._max_step_ftb(s,   ds_aff, float(getattr(self.cfg, "ip_tau", 0.995)))
                a_d_aff = self._max_step_ftb(lmb, dl_aff, float(getattr(self.cfg, "ip_tau", 0.995)))
                mu_aff  = ((s + a_p_aff * ds_aff) @ (lmb + a_d_aff * dl_aff)) / max(1, mI)
                sigma   = (mu_aff / max(mu, 1e-16)) ** float(getattr(self.cfg, "ip_sigma_power", 3.0))
                rhs_corr = (lmb / s) * r_pI + ((sigma * mu) / s - lmb)
                rhs_x = -r_d - ((JI.T @ rhs_corr) if _isspm(JI) else (np.asarray(JI).T @ rhs_corr))
                dx, dnu = solve_KKT(W, rhs_x, JE, r_pE)
                JI_dx = (JI @ dx) if not _isspm(JI) else JI.dot(dx)
                ds = -r_pI - JI_dx
        ...
        a_p = self._max_step_ftb(s,   ds, float(getattr(self.cfg, "ip_tau", 0.995))) if mI > 0 else 1.0
        a_d = self._max_step_ftb(lmb, dlam, float(getattr(self.cfg, "ip_tau", 0.995))) if mI > 0 else 1.0
        ...
        if (self.ls is not None) and (mI > 0):
            ...
            alpha, ls_iters, _needs_restoration = self.ls.search_ip(
                model=model, x=x, dx=dx, ds=ds, s=s, mu=mu, theta0=theta0, alpha_max=alpha_max
            )
        else:
            # fallback simple Armijo on φ
            alpha = alpha_max
            phi0 = float(f - mu * (np.sum(np.log(np.maximum(s, 1e-16))) if s.size else 0.0)
                        + rho * ((np.sum(np.abs(cE)) if cE.size else 0.0) +
            ...
            for _ in range(int(getattr(self.cfg, "ip_ls_max", 25))):
                if alpha < float(getattr(self.cfg, "ip_alpha_min", 1e-10)): break
                s_trial = s + alpha * ds if mI > 0 else s
                if mI > 0 and np.any(s_trial <= 0):
                    alpha *= float(getattr(self.cfg, "ip_alpha_backtrack", 0.5)); ls_iters += 1; continue
                x_trial = x + alpha * dx
                dt = model.eval_all(x_trial, components=["f", "cI", "cE"])
                f_t  = float(dt["f"])
                cI_t = np.zeros(mI) if (mI == 0 or dt["cI"] is None) else np.asarray(dt["cI"], float)
                cE_t = np.zeros(mE) if (mE == 0 or dt["cE"] is None) else np.asarray(dt["cE"], float)
                phi_t = float(f_t - mu * (np.sum(np.log(np.maximum(s_trial, 1e-16))) if s_trial.size else 0.0)
                            + rho * ((np.sum(np.abs(cE_t)) if cE_t.size else 0.0) +
                                    (np.sum(np.abs(cI_t + s_trial)) if (cI_t.size or s_trial.size) else 0.0)))
                if phi_t <= phi0 + float(getattr(self.cfg, "ip_armijo_coeff", 1e-4)) * alpha * dphi:
                    break
                alpha *= float(getattr(self.cfg, "ip_alpha_backtrack", 0.5)); ls_iters += 1

        # -------------------- α == 0 : try SOC rescue, or null-accept μ --------------------
        if alpha <= 0.0:
            # Attempt SOC rescue if enabled
            if use_soc and (soc_when == "after_ls_reject") and (getattr(self, "soc", None) is not None):
                # predictions for Funnel/Filter
                pred_df = max(0.0, float(-(g @ dx)))
                def theta_lin_unit():
                    cE_lin = (cE + (JE @ dx if (mE>0 and JE is not None) else 0.0)) if (mE>0) else None
                ...
                corr, needs_rest = self.soc.compute_correction(
                    model=model, x_trial=x, Delta=Delta,
                    s=(s if mI>0 else None), mu=mu,
                    funnel=getattr(self, "funnel", None),
                    filter=(None if getattr(self, "funnel", None) is not None else getattr(self, "filter", None)),
                    theta0=theta, f0=f, pred_df=pred_df, pred_dtheta=pred_dtheta,
                )
                if (np.linalg.norm(corr) > 0.0) and (not needs_rest):
                    ...
                    if mI > 0 and s.size > 0:
                        ...
                        if np.any(s_new <= 0.0):  # stay interior; drop slack update if needed
                            s_new = s
                    ...
                    if (it + 1) % int(getattr(self.cfg, "ip_mu_reduce_every", 10)) == 0:
                        mu = max(getattr(self.cfg, "ip_mu_min", 1e-12), float(getattr(self.cfg, "ip_mu_reduce", 0.2)) * mu)

                    # pack and return
                    kkt = model.kkt_residuals(x_new, l_new, nu_new)
                    conv = (kkt["stat"] <= getattr(self.cfg, "tol_stat", 1e-8) and
                            kkt["ineq"] <= getattr(self.cfg, "tol_feas", 1e-8) and
                            kkt["eq"]   <= getattr(self.cfg, "tol_feas", 1e-8) and
                            kkt["comp"] <= getattr(self.cfg, "tol_comp", 1e-8) and
                            mu <= getattr(self.cfg, "ip_mu_min", 1e-12))
                    ...
                    info = self._pack_info(
                        ...
                        accepted=True, converged=conv, f=f_new, theta=theta_new,
                        kkt=kkt, alpha=0.0, rho=0.0, mu=mu
                    )
            ...
            kkt_ok = (kkt_now["stat"] <= getattr(self.cfg, "tol_stat", 1e-8) and
                    kkt_now["ineq"] <= getattr(self.cfg, "tol_feas", 1e-8) and
                    kkt_now["eq"]   <= getattr(self.cfg, "tol_feas", 1e-8) and
                    kkt_now["comp"] <= getattr(self.cfg, "tol_comp", 1e-8))
            if allow_null_accept and kkt_ok:
                ...
                conv = (kkt_now["stat"] <= getattr(self.cfg, "tol_stat", 1e-8) and
                        kkt_now["ineq"] <= getattr(self.cfg, "tol_feas", 1e-8) and
                        kkt_now["eq"]   <= getattr(self.cfg, "tol_feas", 1e-8) and
                        kkt_now["comp"] <= getattr(self.cfg, "tol_comp", 1e-8) and
                        mu <= getattr(self.cfg, "ip_mu_min", 1e-12))
                info = self._pack_info(
                    step_norm=0.0, accepted=True, converged=conv,
                    f=float(model.eval_all(x, components=["f"])["f"]),
                    theta=model.constraint_violation(x), kkt=kkt_now,
                    alpha=0.0, rho=0.0, mu=mu
                )
                info["ls_iters"] = ls_iters
            ...
            info = self._pack_info(
                ...
                f=f, theta=theta, kkt={"stat":stat,"ineq":feasI,"eq":feasE,"comp":comp},
                alpha=0.0, rho=0.0, mu=mu
            )
            info["ls_iters"] = ls_iters
            return x, lmb, nuv, info
        ...
        if use_soc and (soc_when == "after_ls_accept") and (getattr(self, "soc", None) is not None):
            # predictions based on accepted step (scale by α)
            pred_df = max(0.0, float(-(g @ dx))) * float(alpha)
            def theta_lin_scaled(v_dx, v_ds):
                cE_lin = (cE + (JE @ v_dx if (mE>0 and JE is not None) else 0.0)) if (mE>0) else None
                inc_I  = (JI @ v_dx if (mI>0 and JI is not None) else 0.0)
                rI_lin = (cI + s + inc_I + (v_ds if mI>0 else 0.0)) if (mI>0) else None
                thE = float(np.sum(np.abs(cE_lin))) if cE_lin is not None else 0.0
                thI = float(np.sum(np.abs(rI_lin))) if rI_lin is not None else 0.0
                return thE + thI
            ...
            corr, needs_rest = self.soc.compute_correction(
                model=model, x_trial=x_trial, Delta=Delta,
                s=(s_trial if mI>0 else None), mu=mu,
                funnel=getattr(self, "funnel", None),
                filter=(None if getattr(self, "funnel", None) is not None else getattr(self, "filter", None)),
                theta0=model.constraint_violation(x_trial),
                f0=float(model.eval_all(x_trial, components=["f"])["f"]),
                pred_df=pred_df, pred_dtheta=pred_dtheta,
            )
            if (np.linalg.norm(corr) > 0.0) and (not needs_rest):
        ...
        info = self._pack_info(
            ...
            accepted=True, converged=conv, f=f_new, theta=theta_new,
            kkt=kkt, alpha=alpha, rho=0.0, mu=mu
        )
        info["ls_iters"] = ls_iters
    ...
    def _max_step_ftb(z: np.ndarray, dz: np.ndarray, tau: float) -> float:
        if z.size == 0 or dz.size == 0: return 1.0
        neg = dz < 0
        if not np.any(neg): return 1.0
        return float(min(1.0, tau * np.min(-z[neg] / dz[neg])))

    @staticmethod
    def _barrier_merit(f: float, mu: float, s: np.ndarray, rho: float, cE: np.ndarray, cI: np.ndarray) -> float:
        term_bar = -mu * (np.sum(np.log(s)) if s.size else 0.0)
        term_pen = rho * ( (np.sum(np.abs(cE)) if cE.size else 0.0) +
                           (np.sum(np.abs(cI + s)) if (cI.size or s.size) else 0.0) )
    ...
    def _pack_info(
        self, step_norm: float, accepted: bool, converged: bool,
        f: float, theta: float, kkt: Dict[str, float],
        alpha: float, rho: float, mu: float,
    ) -> Dict:
        return {
            "mode": "ip",
            "step_norm": step_norm,
