...
from .blocks.reg import DOptStabilizer, Regularizer
...
class SQPStepper:
    ...
    @staticmethod
    def _alpha_max_box(
        x: np.ndarray, p: np.ndarray, lb: Optional[np.ndarray], ub: Optional[np.ndarray]
    ) -> float:
    ...
    def step(
        ...
        if (lb is not None and lb.size != x.size) or (
            ub is not None and ub.size != x.size
        ):
        ...
        H_psd, _ = self.regularizer.regularize(
            H_raw, it, np.linalg.norm(g0), self.tr.delta
        )
        ...
        def kkt_full(g, JE, JI, cE, cI, xval):
            ...
            if (
                JI is not None
                and getattr(JI, "size", 0)
                and lam_user is not None
                and lam_user.size
            ):
                r += JI.T @ lam_user
            r += z_U - z_L
            ...
            feas_in = (
                float(np.linalg.norm(np.maximum(cI, 0.0), ord=np.inf))
                if cI is not None
                else 0.0
            )
            feas_box = 0.0
            if lb is not None:
                feas_box = max(
                    feas_box,
                    float(np.linalg.norm(np.maximum(lb - xval, 0.0), ord=np.inf)),
                )
            if ub is not None:
                feas_box = max(
                    feas_box,
                    float(np.linalg.norm(np.maximum(xval - ub, 0.0), ord=np.inf)),
                )
            ...
            if cI is not None and lam_user is not None and lam_user.size:
                comp = max(
                    comp,
                    float(np.linalg.norm(lam_user * np.maximum(cI, 0.0), ord=np.inf)),
                )
            if lb is not None:
                comp = max(
                    comp,
                    float(np.linalg.norm(z_L * np.maximum(xval - lb, 0.0), ord=np.inf)),
                )
            if ub is not None:
                comp = max(
                    comp,
                    float(np.linalg.norm(z_U * np.maximum(ub - xval, 0.0), ord=np.inf)),
                )
        ...
        converged = (
            kkt["stat"] / g_scale <= self.cfg.tol_stat
            and kkt["ineq"] / g_scale <= self.cfg.tol_feas
            and kkt["eq"] / g_scale <= self.cfg.tol_feas
            and (kkt["comp"] / g_scale <= self.cfg.tol_comp or kkt["ineq"] <= 1e-10)
        ...
        return (
            x_trial,
            (lam_user if lam_user is not None else np.zeros(0)),
            nu_new,
            info,
        )
